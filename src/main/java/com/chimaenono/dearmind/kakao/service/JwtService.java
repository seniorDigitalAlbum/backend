// src/main/java/com/chimaenono/dearmind/kakao/service/JwtService.java
...
public class JwtService {
    ...
    @Value("${jwt.expiration:86400000}") // 24시간 (ms)
    ...
    public String getSubject(String token) {
        try {
            ...
            log.error("JWT 토큰에서 subject 추출 실패", e);
            throw new RuntimeException("유효하지 않은 토큰", e);
        }
    }
    ...
    public long getRemainingSeconds(String token) {
        try {
            ...
            long sec = TimeUnit.MILLISECONDS.toSeconds(Math.max(diffMillis, 0));
